...
__device__ void indexed_moe_forward(
    ...
    // NOTE: must use the template's block width `qk`, not QK_K — for q8_0
    // (32-wide blocks) QK_K gave an 8x-too-small expert stride, so every
    // expert beyond 0 read overlapping/garbage weights.
